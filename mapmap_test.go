...
func TestMapMap(t *testing.T) {
	...
	mm.Delete(-1, -1)
	mm.Delete(0, 1)
	if !reflect.DeepEqual(mm, MapMap[int, int, int]{}) {
...
func TestMapMapCloneAndEqual(t *testing.T) {
	{
		mm1 := MapMap[int, int, int]{}
		mm1.Set(0, 1, 2)

		mm2 := mm1.Clone()

		if !(CompMapMap[int, int, int](mm1).Equal(CompMapMap[int, int, int](mm2))) {
...

	{
		...
		var cmm1 CompMapMap[int, int, int]
		var cmm2 CompMapMap[int, int, int]
		if !cmm1.Equal(cmm2) {
		...
		cmm1 = CompMapMap[int, int, int](mm1)

		if cmm1.Equal(cmm2) {
			t.Fatal("unequal compare the same")
		}

		cmm1 = CompMapMap[int, int, int]{}
		if !cmm1.Equal(cmm2) {
			t.Fatal("nil and empty maps don't say equal")
		}

		cmm1 = CompMapMap[int, int, int](mm1)
		mm2 := MapMap[int, int, int]{}
		mm2.Set(1, 2, 3)

		if cmm1.Equal(CompMapMap[int, int, int](mm2)) {
...

	{
		...
		var mms1 MapMap[int, int, string]
		mms2 := MapMap[int, int, string]{}
...
func TestMapMapEqualFunc(t *testing.T) {
	var mm1 MapMap[int, int, int]
	var mm2 MapMap[int, int, int]
